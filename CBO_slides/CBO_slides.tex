...
\item Journals and funders: Registries (AEA), Journals (Science + 5k other journals), Funders (NIH, NSF and multiple donors)
\end{itemize}
...
\item  Researcher degrees of freedom (Simmons, Nelson, and Simonsohn 2011,Wicherts et al., 2016).
\end{itemize}
\end{frame} 
...
\item \textit{Scalable}: 
\begin{itemize}
\item CBO's reputation: among the most transparent and rigorous policy analysis offices. Lessons from OPA that apply to CBO should apply also to most agencies.
\item Policy issue is widely known which facilitates parallels.
\end{itemize}   
\item \textit{Recurrent}:
\begin{itemize}
\item This policy analysis will be conducted again in the future.
\item Case study can form the basis for future calculations.
\end{itemize}  
\end{itemize}
...
\item \textit{Feasible}: 
\begin{itemize}
\item Available data, good description of the analysis 
\item Only one policy lever to analyze.
\end{itemize}  
\item \textit{Relevant}: 
\end{itemize}
...
\begin{frame}{We Want to Work With You}
\begin{itemize}
...
\item Training research transparency tools.
\end{itemize}
...
\begin{frame}[label=low_tr_ex]{Easier Methodological Appraisal. Example: dis-employment effects \textbf{Before}}
